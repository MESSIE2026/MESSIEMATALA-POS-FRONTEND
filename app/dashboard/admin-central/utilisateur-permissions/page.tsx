...
export default function UtilisateurPermissionsPage() {
  ...
  async function charger() {
    ...
    try {
      const res = await fetch(
        `${API}/utilisateur-permissions/${identreprise}/${idutilisateur}`,
      );

      const data = await res.json();
      setItems(Array.isArray(data) ? data : []);
    } catch {
      setMessage('Erreur chargement permissions utilisateur.');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function changerPermission(item: Permission, value: boolean) {
    ...
    try {
      const res = await fetch(`${API}/utilisateur-permissions/${endpoint}`, {
        method: 'POST',
      ...
      setMessage(value ? 'Permission donnée.' : 'Permission retirée.');
    } catch {
      ...
      <section className="mx-auto max-w-7xl rounded-2xl border border-emerald-900/15 bg-white shadow-xl">
        <div className="border-b border-emerald-900/10 bg-emerald-950 px-6 py-5 text-white">
          ...
          <p className="mt-1 text-sm text-emerald-50">
            Le SuperAdmin donne uniquement les permissions déjà autorisées par le Programmeur.
          </p>
        </div>

        <div className="grid gap-4 border-b border-slate-200 bg-slate-50 p-5 md:grid-cols-5">
          <div>
          ...
          <div>
            ...
            <input
              type="number"
              value={idutilisateur}
              onChange={(e) => setIdutilisateur(Number(e.target.value || 1))}
              className="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3 text-sm font-bold outline-none focus:border-emerald-900"
            />
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <input
              ...
              placeholder="Rechercher permission, module, code..."
                  ...
                  {filtered.map((item) => (
                    <tr key={item.idpermission} className="border-t border-slate-100 hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 text-right">
                        <button
                          onClick={() => changerPermission(item, !item.autorise)}
